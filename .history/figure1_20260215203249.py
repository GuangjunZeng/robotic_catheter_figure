...
def main():
    ...
    tip_dir = smooth_pts[-1] - smooth_pts[-5]  # 尖端切向方向
    ...
    # 在尖端添加虚线框 (ROI)
    roi_box = pv.Box(bounds=[tip_pos[0]-0.3, tip_pos[0]+0.3,
                     tip_pos[1]-0.3, tip_pos[1]+0.3, tip_pos[2]-0.3, tip_pos[2]+0.3])
    plotter.add_mesh(roi_box, color="blue", style="wireframe",
    ...
    # --- 右侧视口: 尖端截面细节 ---
    plotter.subplot(0, 1)
    plotter.add_text("Tip Cross-section View", font_size=12, color="black")

    # 模拟截面视图：
    # 1. 导管截面 (中心圆)
    tip_circle = pv.Disc(center=[0, 0, 0], inner=0,
                         outer=cat_radius, normal=[0, 0, 1])
    plotter.add_mesh(tip_circle, color="#333333", label="Catheter Tip")
    ...
    # 3. 模拟周围障碍物的截面投影 (手动放置以示意)
    # 假设尖端周围有三个红色障碍物投影
    obs_proj1 = pv.Disc(center=[0.15, 0.15, 0],
                        inner=0, outer=obs_size*0.8, normal=[0, 0, 1])
    obs_proj2 = pv.Box(bounds=[-0.25, -0.15, 0.05, 0.25, -0.01, 0.01])  # 侧向障碍物
    ...
    # 设置右侧视口为正交俯视图
    plotter.view_xy()
    plotter.enable_parallel_projection()  # 使用正交投影更像 2D 图纸
