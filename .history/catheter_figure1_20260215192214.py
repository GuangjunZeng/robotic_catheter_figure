...
def create_irregular_obstacle(center, size=0.5, seed=None):
    ...
    # 创建一个基础球体
    sphere = pv.Sphere(radius=size, center=center, theta_resolution=20, phi_resolution=20)
...
def main():
    ...
    # 添加导管到场景，设置颜色和材质
    plotter.add_mesh(catheter, color="red", smooth_shading=True, label="Robotic Catheter", specular=0.5)
    
    # 3. 创建多个不规则障碍物
    obstacles_data = [
        {"center": [0.8, 0.5, 1.0], "size": 0.3, "color": "lightgray", "seed": 42},
        {"center": [1.5, 2.5, 1.8], "size": 0.4, "color": "darkgray", "seed": 123},
        {"center": [0.2, 1.8, 1.2], "size": 0.25, "color": "gray", "seed": 7},
        {"center": [1.8, 1.0, 2.2], "size": 0.35, "color": "silver", "seed": 99},
    ]
    
    for obs in obstacles_data:
        mesh = create_irregular_obstacle(obs["center"], size=obs["size"], seed=obs["seed"])
        plotter.add_mesh(mesh, color=obs["color"], opacity=0.8, smooth_shading=True, specular=0.2)
