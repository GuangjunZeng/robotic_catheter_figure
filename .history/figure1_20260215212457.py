...
def main():
    ...
    # 【方案 A 改进实现】：将导管尾迹投影到截面平面，与障碍物对齐
    # 建立本地坐标系：tip_pos 为原点，tip_dir 为 Z 轴
    z_axis = tip_dir
    if abs(z_axis[0]) < 0.9:
        x_axis = np.array([1, 0, 0]) - np.dot([1, 0, 0], z_axis) * z_axis
    else:
        x_axis = np.array([0, 1, 0]) - np.dot([0, 1, 0], z_axis) * z_axis
    ...
    for i in range(n_segments):
        ...
        for pt in segment_pts:
            rel_vec = pt - tip_pos
            local_x = np.dot(rel_vec, x_axis)
            local_y = np.dot(rel_vec, y_axis)
            proj_pt = np.array([local_x, local_y, tip_pos[2]])
            proj_pts.append(proj_pt)

        proj_pts = np.array(proj_pts)
        segment_poly = pv.PolyData(proj_pts)
        ...
        # 计算颜色：从白色到深灰色
        color_val = int(255 * (1 - progress * 0.8))
