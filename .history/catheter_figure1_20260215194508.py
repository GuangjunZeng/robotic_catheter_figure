...
def main():
    ...
    # 添加导管到场景，设置颜色为黑灰色
    plotter.add_mesh(catheter, color="#333333", smooth_shading=True, label="Robotic Catheter", specular=0.5)
    ...
    for obs in obstacles_data:
        mesh = create_irregular_obstacle(obs["center"], size=obs_size, seed=obs["seed"])
        plotter.add_mesh(mesh, color="red", opacity=0.9, smooth_shading=True, specular=0.3)
    ...
    plotter.add_mesh(small_sphere, color="red", opacity=0.9, smooth_shading=True)
    
    # 3.4 增加一个圆柱体
    cylinder = pv.Cylinder(center=[0.5, 0.8, 0.5], direction=[1, 1, 0], radius=obs_size, height=0.4)
