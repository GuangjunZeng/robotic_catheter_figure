...
def main():
    ...
    # 从外到内渐变：大半径 → 小半径，透明度从浅到深，颜色从浅灰到深灰
    n_rings = 10
    for ring_idx in range(n_rings):
        # 反向计数：外圈是最透明的（后方），内圈是最实的（前方/尖端）
        progress = 1 - (ring_idx / n_rings)  # 1 到 0
        ...
        # 颜色：从浅灰到深灰
        color_val = int(180 + (1 - progress) * 75)  # 从 180 到 255
        color = f"#{color_val:02x}{color_val:02x}{color_val:02x}"

        # 创建圆环（用 Circle 的 tube 表示）
        circle = pv.Circle(radius=ring_radius, resolution=50)
        # 将圆环"变厚"成管状，以体现 3D 感
        ring_tube = circle.tube(radius=cat_radius * 0.08)

        plotter.add_mesh(ring_tube, color=color,
    ...
    # 3. 规范化生成的障碍物（在 XY 平面，Z = tip_pos[2]）
    np.random.seed(42)
    for i in range(15):
