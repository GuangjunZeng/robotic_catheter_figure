...
def main():
    ...
    # 中心点 y 轴同步平移 0.35
    grid_size = 1.0
    base_plane = pv.Plane(center=[0.5, 0.35, 0.7], direction=[0, 1, 0],
    ...
    sphere_pos = [[1.4, 1.74, 2.8]]  # 球的位置
    tetra_pos = [[0.6, 2.21, 2.22]]   # 四面体的位置

    plotter.add_mesh(catheter_mesh, color="#333333",
                     smooth_shading=True, specular=0.5)

    def add_elegant_velocity(plotter, center, direction, size=0.12):
    ...
    for i, pos in enumerate(box_pos):
        ...
        add_elegant_velocity(
            plotter, pos, [-0.01, -0.006, 0.1] if i == 0 else [-0.1, 0.2, -0.1])  # 立方体的方向的箭头位置
    ...
    for i, pos in enumerate(sphere_pos):
        ...
        add_elegant_velocity(
            plotter, pos, [0.1, 0.225, -0.03] if i == 0 else [0.2, 0.1, -0.1])  # 球的方向的箭头位置
    ...
    for i, pos in enumerate(tetra_pos):
        ...
        tetra.rotate_y(62)  # 绕 Y 轴自转 30 度
        # 计算从原点到目标位置的平移向量
