...
def main():
    ...
    # 新增：棱柱（三角形底面）位置
    prism_pos = [[2.2, 1.5, 2.0], [0.8, 1.8, 2.2]]
    ...
    def add_elegant_velocity(plotter, center, direction, size=0.15):
        ...
        plotter.add_mesh(line, color="orange", line_width=2)
        ...
        # 4. 在起点放一个极小的球，增加设计感
        dot = pv.Sphere(radius=size*0.04, center=start_pt)
        plotter.add_mesh(dot, color="orange")
    ...
    # 3. 三角形障碍物 (Prisms)
    for i, pos in enumerate(prism_pos):
        # 创建一个三角形底面的棱柱
        prism = pv.Cylinder(center=pos, direction=[
                            0, 0, 1], radius=obs_size, height=obs_size*2, resolution=3)
        plotter.add_mesh(prism, color="red", style="wireframe",
                         line_width=1, opacity=0.8)
