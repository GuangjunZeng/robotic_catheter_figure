...
def main():
    ...
    # 在尖端添加绿色虚线矩形框 (ROI)，表示截面平面
    # 使用 Plane 创建一个垂直于导管方向的二维矩形
    roi_plane = pv.Plane(center=tip_pos, direction=tip_dir,
                         i_size=0.6, j_size=0.6)
    plotter.add_mesh(roi_plane, color="green",
                     style="wireframe", line_width=2, opacity=0.8)

    # 添加一个半透明的填充平面，增强“截面”的视觉感
    plotter.add_mesh(roi_plane, color="green", opacity=0.1)
    ...
    plotter.add_mesh(roi_border, color="green",
