...
def main():
    ...
    for cap in cylinder_positions:
        cylinder = pv.Cylinder(center=cap["center"], radius=obs_size*0.6, height=obs_size*2.5, direction=cap["dir"])
        plotter.add_mesh(cylinder, color="red", opacity=0.9, smooth_shading=True)
    ...
    for pos in cone_positions:
        cone = pv.Cone(center=pos, direction=[0, 0, 1], radius=obs_size*0.8, height=obs_size*1.5)
